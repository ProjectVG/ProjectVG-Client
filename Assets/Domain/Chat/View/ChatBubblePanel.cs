...
{
	public class ChatBubblePanel : MonoBehaviour
	{
		...
		public void CreateBubble(Actor actor, string text, float displayTime = -1f)
		{
			...
			{
				Debug.LogError("[ChatBubblePanel] ChatBubblePrefab 또는 BubbleContainer가 설정되지 않았습니다!");
				return;
			}
			...
			try
			{
				...
				if (bubbleUI == null)
				{
					Debug.LogError("[ChatBubblePanel] ChatBubbleUI 컴포넌트를 찾을 수 없습니다!");
				...
				{
					Debug.LogWarning($"[ChatBubblePanel] 최대 버블 수({_maxBubbles})에 도달했습니다. 가장 오래된 버블을 제거합니다.");
					RemoveOldestBubble();
				}
			...
			{
				Debug.LogError($"[ChatBubblePanel] 버블 생성 실패: {ex.Message}");
			}
		}
		...
		public void RemoveBubble(ChatBubbleUI? bubble)
		{
			...
			{
				Debug.LogError($"[ChatBubblePanel] 버블 제거 실패: {ex.Message}");
			}
		}
		...
		public void ClearAllBubbles()
		{
			...
			{
				Debug.LogError($"[ChatBubblePanel] 모든 버블 제거 실패: {ex.Message}");
			}
		}
		...
		private void Initialize()
		{
			...
			{
				Debug.LogError("[ChatBubblePanel] BubbleContainer가 설정되지 않았습니다! 인스펙터에서 설정해주세요.");
				return;
			}
			
			if (_chatBubblePrefab == null)
			{
				Debug.LogError("[ChatBubblePanel] ChatBubblePrefab이 설정되지 않았습니다!");
				return;
			}
			
			Debug.Log("[ChatBubblePanel] 초기화 완료");
		}
		...
		private void SetupCanvasGroup(GameObject? bubbleObject)
		{
			...
			if (canvasGroup == null)
			{
				...
				Debug.Log($"[ChatBubblePanel] ChatBubble에 CanvasGroup이 자동으로 추가되었습니다: {bubbleObject.name}");
			}
