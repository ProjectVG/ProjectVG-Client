...
namespace ProjectVG.Infrastructure.Auth.Storage.Platforms
{
    public class IOSSecureStorage : ISecureStorage
    {
        ...
        public string PlatformName => "iOS Keychain";
        ...
        public async UniTask<bool> StoreAsync(string key, string value)
        {
            if (!IsAvailable)
            {
                Debug.LogError("[IOSSecureStorage] Keychain을 사용할 수 없습니다.");
                return false;
            }
            ...
            {
                Debug.LogError($"[IOSSecureStorage] 저장 실패: {ex.Message}");
                return false;
            }
        }
        ...
        public async UniTask<string> LoadAsync(string key)
        {
            ...
            {
                Debug.LogError($"[IOSSecureStorage] 로드 실패: {ex.Message}");
                return null;
            }
        }
        ...
        public async UniTask<bool> DeleteAsync(string key)
        {
            ...
            {
                Debug.LogError($"[IOSSecureStorage] 삭제 실패: {ex.Message}");
                return false;
            }
        }
        ...
        public async UniTask ClearAllAsync()
        {
            ...
            try
            {
                ...
                Debug.Log("[IOSSecureStorage] 모든 데이터 삭제 완료");
            }
            catch (Exception ex)
            {
                Debug.LogError($"[IOSSecureStorage] 전체 삭제 실패: {ex.Message}");
            }
        }
        ...
        public async UniTask<byte[]> LoadEncryptedAsync(string key)
        {
            ...
            {
                Debug.LogError($"[IOSSecureStorage] 바이너리 데이터 디코딩 실패: {ex.Message}");
                return null;
            }
        }
        ...
        private void InitializeKeychain()
        {
            try
            {
                ...
                IsAvailable = InitializeIOSKeychain();
#else
                IsAvailable = false;
                Debug.LogWarning("[IOSSecureStorage] iOS 플랫폼이 아닙니다.");
#endif
            }
            catch (Exception ex)
            {
                Debug.LogError($"[IOSSecureStorage] Keychain 초기화 실패: {ex.Message}");
                IsAvailable = false;
            }
        }
        
        private bool InitializeIOSKeychain()
        {
#if UNITY_IOS && !UNITY_EDITOR
            try
            {
                // TODO: iOS Native Plugin을 통한 Keychain 초기화
                // 1. Keychain Services 접근 권한 확인
                // 2. kSecClass, kSecAttrService 설정
                // 3. Keychain 접근 가능 여부 테스트
                
                Debug.Log("[IOSSecureStorage] iOS Keychain 초기화 성공");
                return true;
            }
            catch (Exception ex)
            {
                Debug.LogError($"[IOSSecureStorage] iOS Keychain 초기화 실패: {ex.Message}");
                return false;
            }
#else
            return false;
        ...
        private async UniTask<bool> StoreInKeychainAsync(string key, string value)
        {
            ...
            try
            {
                // TODO: iOS Native 코드 호출
                // 1. NSString을 NSData로 변환
                ...
                Debug.Log($"[IOSSecureStorage] Keychain 저장 완료: {key}");
                return true;
            }
            catch (Exception ex)
            {
                Debug.LogError($"[IOSSecureStorage] Keychain 저장 실패: {ex.Message}");
                return false;
            }
#else
        ...
        private async UniTask<string> LoadFromKeychainAsync(string key)
        {
            ...
            try
            {
                ...
                // 3. NSData를 NSString으로 변환
                
                await UniTask.Delay(10); // 임시 지연
                Debug.Log($"[IOSSecureStorage] Keychain 로드 완료: {key}");
                return null; // 임시로 null 반환
            }
            catch (Exception ex)
            {
                Debug.LogError($"[IOSSecureStorage] Keychain 로드 실패: {ex.Message}");
                return null;
            }
#else
            await UniTask.CompletedTask;
        ...
        private async UniTask<bool> DeleteFromKeychainAsync(string key)
        {
            ...
            try
            {
                ...
                await UniTask.Delay(10); // 임시 지연
                Debug.Log($"[IOSSecureStorage] Keychain 삭제 완료: {key}");
                return true;
            }
            catch (Exception ex)
            {
                Debug.LogError($"[IOSSecureStorage] Keychain 삭제 실패: {ex.Message}");
                return false;
            }
#else
            await UniTask.CompletedTask;
        ...
        private async UniTask ClearKeychainAsync()
        {
            ...
            try
            {
                ...
                await UniTask.Delay(10); // 임시 지연
                Debug.Log("[IOSSecureStorage] Keychain 전체 삭제 완료");
            }
            catch (Exception ex)
            {
                Debug.LogError($"[IOSSecureStorage] Keychain 전체 삭제 실패: {ex.Message}");
                throw;
            }
#else
            await UniTask.CompletedTask;
